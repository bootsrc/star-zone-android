...
import razerdp.github.com.lib.network.ssl.NetConfig;

/**
 * Created by liushaoming on 2017/3/22.
 * <p>
 * baselib appcontext类
 * <p>
 * 该工具类方法来自：
...
public class AppContext {
    ...
    private static void initSslCertificate() {
        // 添加https证书
        try {
            String[]  certFiles= getAppContext().getAssets().list("certs");
            if (certFiles != null) {
                for (String cert:certFiles) {
                    InputStream is = getAppContext().getAssets().open("certs/" + cert);
