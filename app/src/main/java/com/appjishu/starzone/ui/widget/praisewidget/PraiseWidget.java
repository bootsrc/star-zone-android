package com.appjishu.starzone.ui.widget.praisewidget;
...
/**
 * Created by liushaoming on 2016/2/21.
 * 点赞显示控件
 */
public class PraiseWidget extends TextView {
    ...
    public void setDatas(List<LikeInfo> datas) {
        createSpanStringBuilder(datas);
    }


    private void createSpanStringBuilder(List<LikeInfo> datas) {
        ...
        if (spanStrBuilder == null) {
            ...
            for (int i = 0; i < datas.size(); i++) {
                PraiseClick praiseClick = new PraiseClick.Builder(getContext(), datas.get(i).getUserInfo()).setTextSize(textSize)
                        ...
                        .build();
                try {
                    spanStrBuilder.append(datas.get(i).getUserInfo().getNickname(), praiseClick, 0);
